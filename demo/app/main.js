define(["require", "exports", "esri/WebMap", "esri/views/MapView", "esri/symbols/SimpleMarkerSymbol", "esri/tasks/QueryTask", "esri/tasks/support/Query", "esri/Color", "esri/symbols/SimpleLineSymbol", "esri/layers/GraphicsLayer", "esri/Graphic", "esri/geometry/Polygon", "esri/geometry/Polyline", "esri/geometry/Point", "esri/symbols/SimpleFillSymbol", "esri/geometry/Extent", "esri/geometry/SpatialReference", "esri/widgets/Home", "esri/widgets/ScaleBar", "app/config", "app/HelloWorld", "app/AssetEdit", "app/AssetsList", "dojo/_base/array", "esri/renderers/ClassBreaksRenderer", "esri/PopupTemplate", "esri/core/watchUtils", "fcl/FlareClusterLayer_v4"], function (require, exports, WebMap, MapView, SimpleMarkerSymbol, QueryTask, Query, Color, SimpleLineSymbol, GraphicsLayer, Graphic, Polygon, Polyline, Point, SimpleFillSymbol, Extent, SpatialReference, Home, ScaleBar, config, HelloWorld, AssetEdit, AssetsList, arrayUtils, ClassBreaksRenderer, PopupTemplate, watchUtils, fcl) {
    "use strict";
    ...
    var webmap, mapView, sceneView, activeView, graphics, clusterLayer, clusterLayer_Signs;
    //set some defaults
    ...
    mapView.then(function () {
        ...
        mapView.on("layerview-create", function (event) {
            ...
            if (event.layer.title === "TestWorkTicketLyr") {
                ...
                for (var ind = 0; ind < event.layer.fields.length; ind++) {
        ...
        mapView.on("double-click", function (evt) {
            ...
            var selLyr = getEditLayer();
            if (selLyr == null || selLyr == undefined)
                return;
            if (selLyr.geometryType === "polygon") {
                var geometry = drawPolygon(myPolyLinePnts);
                ...
                var geometry = drawPolyline(myPolyLinePnts);
                editWidget.addAsset(selLyr, geometry);
                $("#viewDiv")[0].style.cursor = "auto";
                // Also clear the points array
                myPolyLinePnts = [];
        ...
        mapView.on("click", function (evt) {
            ...
            if (myWidget != null && myWidget != undefined && myWidget.widgetName === "Manage Assets" && myWidget.addFlag && evt.screenPoint) {
                var selLyr_1 = getEditLayer();
                if (selLyr_1 != null && selLyr_1 != undefined && selLyr_1.title === "TestAssetsLayer") {
        ...
        mapView.on('pointer-move', function (evt) {
            if (editWidget.addFlag == true) {
                var Pnt = mapView.toMap({ x: evt.x, y: evt.y });
                console.log("Pnt.Lat: " + Pnt.latitude + " Pnt.Lon: " + Pnt.longitude);
                var myMapPnt = [Pnt.longitude, Pnt.latitude];
        ...
        watchUtils.whenTrue(mapView, "stationary", function () {
            ...
            if (mapView.extent) {
                var info = "<br> <span> the view extent changed: </span>" +
                    ...
                    mapView.extent.xmax.toFixed(2) +
                    "<br> ymin:" + mapView.extent.ymin.toFixed(2) + " ymax: " +
                    mapView.extent.ymax.toFixed(2);
                console.log(info);
                //clearLayer_Signs();
                // Get PW_Signs features using current view extent
                //querySignsLayerFeatures();
                if (PW_Signs_Created) {
                    var signsLayer = webmap.allLayers.find(function (layer) {
    ...
    function getEditLayer() {
        if (myWidget != null && myWidget != undefined && myWidget.widgetName === "Manage Assets" && editWidget.vis) {
            var editLyr = editWidget.getEditLayer();
            return editLyr;
        }
        return null;
    }
    ...
    function queryWTLayerFeatures() {
        ...
        var wtLayer = webmap.allLayers.find(function (layer) {
    ...
    function addClusters(resp) {
        wtInfo.data = arrayUtils.map(resp, function (p) {
    ...
    function addClusters_Signs(resp) {
        signsInfo.data = arrayUtils.map(resp, function (p) {
            //var latlng = new  Point(parseFloat(p.Lon), parseFloat(p.Lat), wgs);
            //var webMercator = webMercatorUtils.geographicToWebMercator(latlng);
            var attributes = {
                "FID": p.FID,
